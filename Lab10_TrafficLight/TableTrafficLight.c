...
#define MASK_PORTB 0x1F 							// Pins 0,1,2,3,4,5
...
#define S_wa 3 //Walk
#define S_hu 4 //Hurry
#define S_gn 5 //North green
#define S_yn 6 //North yellow
...
/*	No		e			n			en		p			pe		pn		pne */
	...
	{RR,W,50,{S_hu,	S_hu,	S_hu,	S_hu,	S_wa,	S_hu,	S_hu,	S_hu}},//Walk
	{RR,H,10,{S_ge,	S_ge,	S_gn,	S_ge,	S_wa,	S_ge,	S_gn,	S_ge}},//Hurry - Moves to ge
...
void set_lights_and_wait(int state){
	...
	GPIO_PORTB_DATA_R = (GPIO_PORTB_DATA_R & (~0x3F)) | fState.out;
	if (fState.out_walk == H){
		GPIO_PORTF_DATA_R = (GPIO_PORTF_DATA_R & (~H)) | S;
		//SysTick_Wait10ms(FSM[state].time/4);
		GPIO_PORTF_DATA_R = (GPIO_PORTF_DATA_R & (~H)) ^ S;
		//SysTick_Wait10ms(FSM[state].time/4);
		GPIO_PORTF_DATA_R = (GPIO_PORTF_DATA_R & (~H)) ^ S;
		//SysTick_Wait10ms(FSM[state].time/4);
		...
		GPIO_PORTF_DATA_R = (GPIO_PORTF_DATA_R & (~H)) | fState.out_walk;
